...
class Enemy(pg.sprite.Sprite):
    def __init__(self, coord_x, coord_y, frame_rate=100, speed_walk=6, gravity=30, life=100):
        super().__init__()
        self.__iddle_r = sf.get_surface_from_spritesheet('models/enemy/enemy.png', 10, 1, flip=True)
        self.__iddle_l = sf.get_surface_from_spritesheet('models/enemy/enemy.png', 10, 1)
        self.__walk_r = sf.get_surface_from_spritesheet('models/enemy/enemy.png', 10, 1,flip=True)
        self.__walk_l = sf.get_surface_from_spritesheet('models/enemy/enemy.png', 10, 1)

        self.__life = life
        self.__direction_change_counter = 0
        self.__direction_change_delay = 1
        self.__move_x = 0
        self.__move_y = 0
    ...
    def do_movement(self, delta_ms, plataformas):
        ...
        if self.__enemy_move_time >= self.__frame_rate:
            ...
            if random.randint(1, 50) % 2 == 0 and self.__direction_change_counter == 0:
                ...
                self.__direction_change_counter = 10
